...
from imagesearch.detection_helpers import sliding_window
from imagesearch.detection_helpers import image_pyramid
import numpy as np
import argparse
import imutils
import time
...
# orig = cv2.imread("NoFilterModeDataSet/Aboard1.png")
# orig = imutils.resize(orig, width=WIDTH)
# (H, W) = orig.shape[:2]

imcap = cv2.VideoCapture(0)
imcap.set(3, 200)  # set width as 640
imcap.set(4, 200)
# orig = imutils.resize(imcap, width=WIDTH)
# (H, W) = orig.shape[:2]
...
def detect(orig, H=200, W=200):
    ...
    start = time.time()
    preds = loaded_model.predict(rois)
    end = time.time()
    ...
    p = [np.argmax(r) for r in prediction]
...
while True:
    ...
    orig = imutils.resize(imgGray, width=WIDTH)
    (H, W) = orig.shape[:2]
    detect(orig)
